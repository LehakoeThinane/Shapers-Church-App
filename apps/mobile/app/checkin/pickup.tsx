...
export default function PickupScreen() {
  ...
  async function onConfirm(checkinId: string, qrToken: string) {
    ...
    setLoading(true);
    try {
      await confirmPickup(getSupabaseClient(), checkinId, qrToken.trim());
      setActiveId(null);
      setToken("");
      await load();
    } catch (err) {
                ...
                {scanning && permission?.granted ? (
                  ...
                  >
                    <CameraView
                      ...
                      onBarcodeScanned={(event) => onConfirm(checkin.id, event.data)}
                    />
                  </View>
                ) : (
                  <Button
